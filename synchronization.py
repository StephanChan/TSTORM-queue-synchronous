...
class Lines(module.Module):
    ...
    def lists(self):
        ...
        max=np.max(len(self.list_647),len(self.camera_list))
        self.list_405.extend([0]*(max-len(self.list_405)))
        self.list_647.extend([0]*(max-len(self.list_647)))
        self.camera_list.extend([0]*(max-len(self.camera_list)))
        self.blk_list=[1]*max
    ...
    def run(self,step):
            ...
            self.task.StartTask()
            while( not self.task.IsTaskDone(ctypes.byref(self.done))):
                time.sleep(1)
            self.stop()
